...
$uip = explode(',',$uip); // Au cas ou X_FORWARDED_FOR rend plusieurs ip on prend la 1er qui est la vrai client ip
...
function secure(){
    ...
    if ($id_de_session != null && $id_user != null) {
        ...
        if ($row['date'] > $time - $timesession && $row['ip'] != $user_ip)
            $secu_user = 0;
        if ($secu_user  == 1) {
			if ($row['ip'] != $user_ip) $check_proxy = checkproxy(); // Si pas la meme ip on check
			...
			// Yoken : On update egalement l'ip qui aura ete verifier juste avant
        ...
        // Incorect session information
			...
			// On cree un cookie ip pour ne ne pas reverifier a chaque session expire
...
function init_cookie() {
	...
	setcookie($cookie_userip, ''); // On vide le cookie ip durant la session (securite pour les users)
...
function session_new($userid, $remember_me) {
	...
	// Si ip cookie differente, on verifie
