...
function affich_block_slideshow($blok){
	...
	if($active == "1"){
		...
		echo'	<link rel="stylesheet" href="media/css/slide_center.css">';
	}
...
<?php	
    ...
    $sql = mysql_query("SELECT sid, titre, url FROM " . GALLERY_TABLE . " ORDER BY sid DESC LIMIT 0, 5");
    while (list($sid, $titre, $url) = mysql_fetch_array($sql))
	{ 
        ...
        $image = "<img src=\"" . $img . "\" " . $width . " alt=\"" . $titre . "\" title=\"" .  _CLICTOSCREEN . "\" />";
		
		
        $blok['content'] .= '<div id="img" class="slide"><a href="' . $url . '" rel="shadowbox" title="' . $titre . '">' . $image . '</a><a href="index.php?file=Gallery&op=description&sid=' . $sid . '" title="' . _VOIR . '&nbsp;' . $titre . '" ><div class="caption" style="bottom:0">' . $titre . '</div></a></div>';
	} 
...
function edit_block_slideshow($bid){
    global $nuked, $language;
